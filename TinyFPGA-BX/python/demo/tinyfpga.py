...
def boot():
	...
	if (portValue != None):
		...
		print(f"{bcolors.WARNING}TinyFPGA not found!{bcolors.ENDC}")
...
def lfsr_read(ser):
	...
	lfsr = ser.read(4)
	if (len(lfsr) < 4):
		print(f"{bcolors.WARNING}{4-len(lfsr)} LFSR bytes are missing{bcolors.ENDC}")
	else:
...
def outdata(data, ser):
	...
	if (len(crc) < 4):
		print(f"{bcolors.WARNING}{4-len(crc)} CRC32 bytes are missing{bcolors.ENDC}")
	elif (int.from_bytes(crc, "little") != crc32):
		print(f"{bcolors.WARNING}CRC error: CRC32={hex(crc32)} should be {hex(int.from_bytes(crc, 'little'))}{bcolors.ENDC}")
	elif (end != start):
...
def indata(length, ser, verbose=0):
	...
	if (len(data) < length+4):
		print(f"{bcolors.WARNING}{length+4-len(data)} bytes are missing{bcolors.ENDC}")
	elif (binascii.crc32(data) != 0x2144df1c):
		print(f"{bcolors.WARNING}CRC error: CRC32={hex(binascii.crc32(data))} should be {hex(0x2144df1c)}{bcolors.ENDC}")
	elif (end != start):
		print(f"{bcolors.OKGREEN}OK! {length} bytes transferred in {round(end-start, 3)} sec ({round(length/1000/(end-start), 1)} kB/s){bcolors.ENDC}")
	else:
		print(f"{bcolors.OKGREEN}OK! {length} bytes transferred in {round(end-start, 3)} sec{bcolors.ENDC}")
...
def rom_read(length, ser, verbose=0):
	...
	if (verbose == 1):
		...
		print(dump.data_dump(data))
	if (len(data) < length):
		print(f"{bcolors.WARNING}{length-len(data)} bytes are missing{bcolors.ENDC}")
	elif (end != start):
		print(f"{bcolors.OKGREEN}OK! {length} bytes transferred in {round(end-start, 3)} sec ({round(length/1000/(end-start), 1)} kB/s){bcolors.ENDC}")
	else:
		print(f"{bcolors.OKGREEN}OK! {length} bytes transferred in {round(end-start, 3)} sec{bcolors.ENDC}")
	...
	if (data[0:1024] != ROM_DATA):
		print(f"{bcolors.WARNING}ERROR: ROM data mismatch!{bcolors.ENDC}")
